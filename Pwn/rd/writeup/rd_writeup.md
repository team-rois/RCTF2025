...
- 注销⽤⼾的时候只 free 了 user 结构体，没有清理 user 结构体的成员变量
- 为 user 分配 task 结构体时，如果 task 数量满则提前返回，此时未给 user->tasks 结构体初始化，造
  成变量未初始化的漏洞，结合上⼀处的 free(user) 清理不完全，可以导致新申请的 user 结构体复⽤旧
  的 user 结构体；
- 条件竞争，由于可以实现 2 个 user 共享同⼀个 task 结构体，在多线程的 do\_task 任务⾥存在
  task\_log 这⼀⻓时间的竞争窗⼝可以实现条件竞争。
